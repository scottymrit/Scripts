...
for host in `for f in $hosts; do echo $f | sed -e 's/\.int.westgroup.com//' ; done`; do
    ...
    rm -r /wca/tomcat.${host}/webapps/wca-webservices
    rm -r /wca/tomcat.${host}/webapps/wca-webservices.war
    cp /wca/wca-deploy/wca-webservices.war /wca/tomcat.${host}/webapps/
